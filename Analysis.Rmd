...
# scatterplot showing plate_x and plate_z, with the strike zone overlaid, and alpha to show density, with the rectangle just an outline
...
```{r strikezone diff}
# calculate strike zone area for current and ABS strike zones
full_pitch_df <- full_pitch_df %>% mutate(sz_area = sz_height * 17, abs_sz_area = abs_sz_height * 17)

# calculate % difference in strike zone area
full_pitch_df <- full_pitch_df %>% mutate(sz_diff = (sz_area - abs_sz_area) / sz_area)

# calculate BABIZ for current and ABS strike zones
full_pitch_df <- full_pitch_df %>% mutate(in_zone = ifelse(plate_z > sz_bot & plate_z < sz_top & plate_x > -8.5 & plate_x < 8.5, 1, 0))

sz_babiz <- full_pitch_df %>% filter(in_zone == 1) %>% group_by(batter_name) %>% summarise(babiz = mean(ifelse(events == "Single" | events == "Double" | events == "Triple" | events == "Home Run", 1, 0)))

abs_sz_babiz <- full_pitch_df %>% filter(in_zone == 1) %>% group_by(batter_name) %>% summarise(babiz = mean(ifelse(events == "Single" | events == "Double" | events == "Triple" | events == "Home Run", 1, 0)))

# calculate BABIP for current and ABS strike zones

sz_babip <- full_pitch_df %>% filter(in_zone == 1) %>% group_by(batter_name) %>% summarise(babip = mean(ifelse(events != "Strikeout" & events != "Walk" & events != "Hit By Pitch", 1, 0)))

abs_sz_babip <- full_pitch_df %>% filter(in_zone == 1) %>% group_by(batter_name) %>% summarise(babip = mean(ifelse(events != "Strikeout" & events != "Walk" & events != "Hit By Pitch", 1, 0)))

# looking at difference in babiz and babip for current and ABS strike zones

babiz_diff <- sz_babiz %>% full_join(abs_sz_babiz, by = "batter_name") %>% mutate(babiz_diff = babiz.x - babiz.y)

babip_diff <- sz_babip %>% full_join(abs_sz_babip, by = "batter_name") %>% mutate(babip_diff = babip.x - babip.y)
